...
- [Horizontale automatische Skalierung](#horizontal-auto-scaling) (Nur für skalierte Architektur verfügbar) - Fügt Webserver-Knoten für Projekte mit skalierter Architektur hinzu oder entfernt sie.
- [Vertikale automatische Skalierung](#vertical-auto-scaling) (für Standard Pro-Architektur oder skalierte Architektur verfügbar) - Passt die CPU-Kapazität vorhandener Knoten an geänderte Anforderungen an.
...
Zusätzlich zur herkömmlichen [horizontalen automatischen Skalierung](#auto-scaling) bietet [!DNL Adobe Commerce on cloud infrastructure] auch vertikale automatische Skalierung für standardmäßige Pro-Architektur- und skalierte Architekturprojekte.

Anstatt Knoten hinzuzufügen oder zu entfernen, ändert die vertikale automatische Skalierung die CPU-Kapazität vorhandener Knoten, um Nachfrageänderungen zu berücksichtigen. Dies ergänzt die horizontale automatische Skalierung, mit der Webserverknoten für Projekte mit skalierter Architektur hinzugefügt oder entfernt werden.

- **Knoten hinzugefügt**: Nicht zutreffend. Die vertikale automatische Skalierung ändert die Größe vorhandener Knoten, anstatt neue hinzuzufügen.
- **Knotengröße**: Die Größe eines Knotens wird auf die nächstgrößere Instanzgröße geändert, wenn der Speicherdruck den definierten Schwellenwert überschreitet. Pro Skalierungsereignis wird nur eine Größenerhöhung angewendet.
- **Knotenverkleinerung**: Die Größe von Knoten wird automatisch verkleinert, wenn die Nachfrage nachlässt. Mindest- und Höchstgrößen werden anhand des Nutzungsmusters eines jeden Projekts und vertraglich festgelegter Ressourcenbeschränkungen festgelegt, wodurch das Risiko unnötiger Skalierung verringert wird.

### Schwellenwerte für automatische Skalierung

Vertikale automatische Skalierungsereignisse werden mithilfe von PSI (Pressure Stall Information) für Speicher unter Linux ausgelöst, das misst, wie viel Zeit ein System aufgrund des Speicherdrucks verbringt. Adobe legt Schwellenwerte auf Grundlage der vertraglich vereinbarten Ressourcenbeschränkungen und Nutzungsmuster Ihres Projekts fest. Händler können diese derzeit nicht konfigurieren.

### Überwachen von Schwellenwerten mit New Relic

Sie können den [!DNL New Relic]-Service verwenden, um Details von Infrastrukturinstanzen zu überwachen, einschließlich Größe und Typ der Instanz. Konfigurieren Sie Warnhinweise in New Relic, die benachrichtigt werden, wenn eine Instanz durch ein vertikales Auto-Scaling-Ereignis geändert wird.

### Auswirkungen auf Ihre Umgebung

Die vertikale automatische Skalierung hat folgende Auswirkungen auf Ihre Umgebung:

- **Ausfallzeit**: Wenn die Größe eines Knotens geändert wird, sind keine Ausfallzeiten zu erwarten.
- **Timing**: Die Größenanpassung eines Knotens dauert in der Regel 20-30 Minuten. Der Knoten wird vorübergehend aus dem Lastenausgleich genommen, während die Größenänderung ausgeführt wird.
